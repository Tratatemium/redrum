...
function isSolved() {
  ...
  return pieces.every((piece) => {
    ...
    const currentDegrees = THREE.MathUtils.radToDeg(
      piece.object3D.rotation[component.data.axis],
    );
  ...
  init: function () {
    ...
    this.el.addEventListener("click", () => {
      ...
      const targetRotation =
        currentRotation - THREE.MathUtils.degToRad(this.data.degrees);
      ...
      gsap.to(this.el.object3D.rotation, {
        ...
        onComplete: () => {
          ...
          const normalizedRotation =
            ((rotationRadians % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
          this.el.object3D.rotation[this.data.axis] = normalizedRotation;

          if (isSolved()) {
            solved = true;
            this.el.sceneEl.emit("puzzle-1-solved");
            document
